...
struct TimingStats {
  ...
  void print() {
    ...
    std::cout << "  Grayscale Conversion: " << std::setw(8)
              << grayscale_conversion << " ms" << std::endl;
    std::cout << "  Pyramid Creation:     " << std::setw(8) << pyramid_creation
              << " ms" << std::endl;

    std::cout << "\nKeypoint Detection:" << std::endl;
    for (size_t i = 0; i < level_times.size(); i++) {
      std::cout << "  Level " << i << " Processing:   " << std::setw(8)
                << level_times[i] << " ms" << std::endl;
    }
    std::cout << "  Detector Inference:   " << std::setw(8)
              << detector_inference << " ms" << std::endl;
    std::cout << "  NMS Processing:       " << std::setw(8) << nms_processing
              << " ms" << std::endl;
    std::cout << "  Keypoint Sorting:     " << std::setw(8) << keypoint_sorting
              << " ms" << std::endl;

    std::cout << "\nDescriptor Computation:" << std::endl;
    std::cout << "  Patch Extraction:     " << std::setw(8) << patch_extraction
              << " ms" << std::endl;
    std::cout << "  Descriptor Inference: " << std::setw(8)
              << descriptor_inference << " ms" << std::endl;

    float total = grayscale_conversion + pyramid_creation + detector_inference +
                  nms_processing + keypoint_sorting + patch_extraction +
                  descriptor_inference;
    std::cout << "\nTotal Time:            " << std::setw(8) << total << " ms"
              << std::endl;
    std::cout << "========================" << std::endl;
  }
};
...
KeyNetInference::KeyNetInference(const std::string &detector_model_path,
                                 ...
                                 int nms_size, int patch_size, float s_mult,
                                 float nms_threshold, int batch_size_desc,
                                 bool use_gpu)
    : num_levels(num_levels), scale_factor(scale_factor), nms_size(nms_size),
      patch_size(patch_size), s_mult(s_mult), nms_threshold(nms_threshold),
      batch_size_desc(batch_size_desc),
      env(ORT_LOGGING_LEVEL_WARNING, "KeyNetInference"),
      memory_info(
          Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeCPUOutput)),
      input_names{"input"}, output_names{"output"} {

  // Initialize ONNX sessions with improved provider configuration
  Ort::SessionOptions session_options;

// Enable verbose logging to see provider assignments (only in debug builds)
#ifdef _DEBUG
  session_options.SetLogSeverityLevel(ORT_LOGGING_LEVEL_VERBOSE);
    ...
    // Add CUDA provider if use_gpu is true and CUDA is available
if (use_gpu && cv::cuda::getCudaEnabledDeviceCount() > 0) {
    std::cout << "Configuring for GPU inference..." << std::endl;
    // Set execution provider options for CUDA
    OrtCUDAProviderOptions cuda_options;
    cuda_options.device_id = 0;
    cuda_options.arena_extend_strategy = 0;              // Default strategy
    // cuda_options.gpu_mem_limit = 2 * 1024 * 1024 * 1024; // 2GB limit
    cuda_options.cudnn_conv_algo_search = OrtCudnnConvAlgoSearchExhaustive;
    cuda_options.do_copy_in_default_stream = 1;

    // Append CUDA provider with options
    session_options.AppendExecutionProvider_CUDA(cuda_options);
} else {
std::cout << "Configuring for CPU inference..." << std::endl;
}

  // Register custom ops if needed
  // session_options.RegisterCustomOpsLibrary("path/to/custom_ops.dll");

  std::cout << "Creating detector session..." << std::endl;
  detector_session = std::make_unique<Ort::Session>(
      env, detector_model_path.c_str(), session_options);

  std::cout << "Creating HyNet session..." << std::endl;
  hynet_session = std::make_unique<Ort::Session>(env, hynet_model_path.c_str(),
                                                 session_options);

  // Print information about input and output nodes
  ...
  // Print detector model info
  {
    ...
    for (size_t i = 0; i < num_input_nodes; i++) {
      ...
      Ort::TypeInfo type_info = detector_session->GetInputTypeInfo(i);
      auto tensor_info = type_info.GetTensorTypeAndShapeInfo();
      std::vector<int64_t> input_dims = tensor_info.GetShape();

      std::cout << "Input " << i << ": name=" << input_name.get() << ", dims=[";
      ...
      // input_name will be automatically freed when it goes out of scope
    }
  }

  // Perform warmup inference to initialize GPU resources
  std::cout << "Warming up models..." << std::endl;

  // Create small dummy input for detector
  ...
  // Run detector warmup inference
  std::cout << "Running detector warmup inference..." << std::endl;
  detector_session->Run(Ort::RunOptions{nullptr}, input_names.data(),
                        &input_tensor, 1, output_names.data(), 1);

  // Warmup HyNet (descriptor network) with dummy patch
  cv::Mat dummy_patch(patch_size, patch_size, CV_32F, cv::Scalar(0.5));
  std::vector<float> patch_tensor_values(dummy_patch.total());
  memcpy(patch_tensor_values.data(), dummy_patch.ptr<float>(),
         dummy_patch.total() * sizeof(float));

  std::vector<int64_t> patch_dims = {1, 1, static_cast<int64_t>(patch_size),
                                     static_cast<int64_t>(patch_size)};

  Ort::Value patch_tensor = Ort::Value::CreateTensor<float>(
      memory_info, patch_tensor_values.data(), patch_tensor_values.size(),
      patch_dims.data(), patch_dims.size());

  // Run HyNet warmup inference
  std::cout << "Running HyNet warmup inference..." << std::endl;
  hynet_session->Run(Ort::RunOptions{nullptr}, input_names.data(),
                     &patch_tensor, 1, output_names.data(), 1);

  std::cout << "Models warmed up successfully" << std::endl;

  // Check execution providers
  try {
    // Use the modern API for getting available providers
    auto available_providers = Ort::GetAvailableProviders();

    std::cout << "\nAvailable execution providers ("
              << available_providers.size() << "):" << std::endl;
    for (const auto &provider : available_providers) {
      std::cout << "  " << provider << std::endl;
    }
  } catch (const Ort::Exception &e) {
    std::cout << "Failed to query available providers: " << e.what()
              << std::endl;
  }
}
...
void KeyNetInference::extractFeatures(const cv::Mat &input_image,
  ...
  timing.keypoint_sorting =
      std::chrono::duration_cast<std::chrono::milliseconds>(t10 - t9).count();

  // timing.print();
}

cv::Mat KeyNetInference::extractDescriptorPatches(
  ...
  if (!patches.empty()) {
    ...
    stacked_patches =
        stacked_patches.reshape(1, patches.size()); // Reshape to [N, 1, H, W]
  }
...
cv::Mat KeyNetInference::computeHyNetDescriptors(const cv::Mat &patches) {
  ...
  for (int i = 0; i < total_patches; i += batch_size_desc) {
    ...
    memcpy(input_tensor_values.data(), float_batch.ptr<float>(),
           float_batch.total() * sizeof(float));
    ...
    cv::Mat batch_descriptors(current_batch_size, output_shape[1], CV_32F,
                              output_data);
    cv::Mat batch_descriptors_clone =
        batch_descriptors.clone(); // Clone to own the data
...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    std::cout
        << "Usage: " << argv[0]
        << " <path_to_detector_model> <path_to_hynet_model> <path_to_image>"
        << std::endl;
    return 1;
  }
  ...
  std::cout << "CUDA devices: " << cv::cuda::getCudaEnabledDeviceCount()
            << std::endl;
